...
# -*- coding: utf-8 -*-
"""
🩺 IR通道PTT峰值检测器 - 时域心率与真实值均值比较版本

核心改进：
- 30s滑动窗口分析
- 每个窗口的时域心率与真实值（从biopac文件获取）对比验证
...
class IRWindowedPTTPeakDetector:
    ...
    def __init__(self, root_path="/root/autodl-tmp/", method="auto"):
        ...
        self.window_step = 5       # 5秒滑窗步长
        self.hr_tolerance_bpm = 5  # 时域心率与真实值差异容忍度
        
        # 选择峰值检测方法
        self.detection_method = self._select_method(method)
        print(f"🔧 峰值检测方法: {self.detection_method}")
        print(f"🪟 窗口参数: {self.window_duration}s窗口, {self.window_step}s滑窗步长")
        print(f"🎯 验证标准: 时域心率与真实值差异<{self.hr_tolerance_bpm}BPM")
    ...
    def get_true_hr_from_biopac(self, window_start_s, window_end_s, biopac_data):
        """从biopac数据中获取窗口内的真实心率均值"""
        try:
            if biopac_data is None or 'hr' not in biopac_data.columns:
                return 0.0
            
            # 筛选窗口时间范围内的数据
            window_mask = (biopac_data.index >= window_start_s) & (biopac_data.index <= window_end_s)
            window_hr_data = biopac_data.loc[window_mask, 'hr']
            
            # 过滤有效的心率值（50-200 BPM）
            valid_hr = window_hr_data[(window_hr_data >= 50) & (window_hr_data <= 200)]
            
            if len(valid_hr) > 0:
                mean_hr = np.mean(valid_hr)
                # print(f"   窗口 {window_start_s:.1f}s-{window_end_s:.1f}s: 找到{len(valid_hr)}个有效心率值，均值={mean_hr:.1f}BPM")
                return mean_hr
            else:
                print(f"   窗口 {window_start_s:.1f}s-{window_end_s:.1f}s: 没有找到有效心率值")
                return 0.0
                
        except Exception as e:
            print(f"⚠️  真实心率计算失败: {e}")
            return 0.0
    ...
    def load_biopac_data(self, subject, exp_id):
        """加载biopac数据获取真实心率"""
        try:
            biopac_file = os.path.join(self.data_path, f"{subject}_{exp_id}_biopac_aligned.csv")
            if not os.path.exists(biopac_file):
                print(f"⚠️  Biopac文件不存在: {biopac_file}")
                return None
            
            df = pd.read_csv(biopac_file)
            if 'hr' not in df.columns:
                print(f"⚠️  Biopac文件中没有hr列")
                return None
            
            # 设置时间索引 - 将Unix时间戳转换为相对时间（秒）
            if 'timestamp' in df.columns:
                # 将时间戳转换为相对时间（从0开始）
                timestamps = df['timestamp'].values
                relative_times = timestamps - timestamps[0]  # 从0开始的相对时间
                df['relative_time'] = relative_times
                df.set_index('relative_time', inplace=True)
                print(f"📊 时间范围: {relative_times[0]:.1f}s - {relative_times[-1]:.1f}s")
            else:
                # 如果没有时间戳，使用采样点索引
                df.index = np.arange(len(df)) / 100  # 假设100Hz采样率
            
            print(f"✅ 加载Biopac数据: {len(df)}条记录，心率范围: {df['hr'].min():.1f}-{df['hr'].max():.1f} BPM")
            return df
            
        except Exception as e:
            print(f"❌ 加载Biopac数据失败: {e}")
            return None
    
    def analyze_sensor_windowed(self, signal, sensor_name, fs, biopac_data):
        """对单个传感器进行窗口化分析"""
        ...
        for window in windows:
            ...
            # print(window['start_time_s'], window['end_time_s'])
            
            # 从biopac数据获取真实心率均值
            true_hr = self.get_true_hr_from_biopac(
                window['start_time_s'], window['end_time_s'], biopac_data
            )
            
            # 时域心率与真实值一致性验证
            hr_diff = abs(peak_result['peak_hr_bpm'] - true_hr)
            is_valid = (
    ...
    def match_peaks_across_sensors_windowed(self, sensor_window_results):
        ...
        try:
            ...
            for window_id, sensor_data in valid_multi_sensor_windows.items():
                # 选择峰值最多的传感器作为参考
                reference_sensor = max(sensor_data.keys(), 
                                     key=lambda s: sensor_data[s]['peak_count'])
                reference_peaks = sensor_data[reference_sensor]['global_peak_times']
                
                # 为每个参考峰值匹配其他传感器的峰值
                for ref_time in reference_peaks:
                    heartbeat_data = {
                        'heartbeat_id': heartbeat_id,
                        'window_id': window_id,
                        'reference_sensor': reference_sensor,
                        'window_start_s': sensor_data[reference_sensor]['start_time_s'],
                        'window_end_s': sensor_data[reference_sensor]['end_time_s']
                    }
                    
                    sensors_matched = 0
                    for sensor, window_data in sensor_data.items():
                        sensor_peaks = window_data['global_peak_times']
                        
                        # 在±0.2秒内查找最近的峰值
                        if len(sensor_peaks) > 0:
                            time_diffs = np.abs(sensor_peaks - ref_time)
                            closest_idx = np.argmin(time_diffs)
                            
                            if time_diffs[closest_idx] <= 0.2:  # 200ms窗口
                                heartbeat_data[f'{sensor}_peak_time_s'] = sensor_peaks[closest_idx]
                                heartbeat_data[f'{sensor}_peak_idx'] = window_data['global_peak_indices'][closest_idx]
                                sensors_matched += 1
                            else:
                                heartbeat_data[f'{sensor}_peak_time_s'] = np.nan
                                heartbeat_data[f'{sensor}_peak_idx'] = np.nan
                        else:
                            heartbeat_data[f'{sensor}_peak_time_s'] = np.nan
                            heartbeat_data[f'{sensor}_peak_idx'] = np.nan
                    
                    # 只保留至少有2个传感器匹配的心跳
                    if sensors_matched >= 2:
                        matched_heartbeats.append(heartbeat_data)
                        heartbeat_id += 1
            
            print(f"💓 匹配的心跳数量: {len(matched_heartbeats)}")
    ...
    def process_experiment(self, subject, exp_id):
        """处理单个实验的窗口化分析"""
        print(f"\n🔍 开始处理 {subject} 实验 {exp_id} - 时域心率与真实值均值比较")
        ...
        # 加载biopac数据获取真实心率
        biopac_data = self.load_biopac_data(subject, exp_id)
        ...
        for sensor in self.sensors:
            try:
                ...
                window_results, valid_windows = self.analyze_sensor_windowed(
                    ir_signal, self.sensor_mapping[sensor], current_fs, biopac_data
                )
    ...
    def save_windowed_results(self, exp_id, sensor_window_results, matched_results, sensor_signals):
        ...
        try:
            ...
            for sensor, windows in sensor_window_results.items():
                for window in windows:
                    mean_ibi = np.mean(window['ibi_ms']) if len(window['ibi_ms']) > 0 else np.nan
                    window_summary.append({
                        ...
                        'true_hr_bpm': window['true_hr_bpm'],
                    ...
                    if window['is_valid']:
                        ...
                        for i, (peak_time, peak_idx) in enumerate(zip(
                            ...
                            valid_peaks.append({
                                ...
                                'window_true_hr_bpm': window['true_hr_bpm'],
            ...
            # 3. 匹配的心跳和PTT计算
            if matched_results and 'matched_heartbeats' in matched_results:
                heartbeat_data = matched_results['matched_heartbeats']
                
                if heartbeat_data:
                    heartbeat_df = pd.DataFrame(heartbeat_data)
                    heartbeat_file = os.path.join(self.current_exp_output_dir, f"matched_heartbeats_windowed_exp_{exp_id}.csv")
                    heartbeat_df.to_csv(heartbeat_file, index=False)
                    print(f"💾 保存窗口化匹配心跳: {heartbeat_file}")
                    
                    # 计算PTT
                    self.calculate_windowed_ptt(heartbeat_df, exp_id, sensor_window_results)
    ...
    def calculate_windowed_ptt(self, heartbeat_df, exp_id, sensor_window_results):
        ...
        try:
            ...
            sensors = [col.replace('_peak_time_s', '') for col in sensor_cols]
            
            if len(sensors) < 2:
                print("⚠️  传感器数量不足，无法计算PTT")
                return
                    ...
                    if col1 in heartbeat_df.columns and col2 in heartbeat_df.columns:
                        ...
                        for _, row in valid_data.iterrows():
                            ...
                            ptt_data.append({
                                ...
                                'reference_mean_ibi_ms': ref_mean_ibi
                            })
            ...
            for idx, sensor in enumerate(self.sensors[:4]):
                ...
                if sensor in sensor_window_results:
                    ...
                    if valid_windows:
                        ...
                        true_hrs = [w['true_hr_bpm'] for w in valid_windows]
                        ax.scatter(peak_hrs, true_hrs, color='green', alpha=0.7, s=50, 
                                 label=f'Valid ({len(valid_windows)})')
                    
                    # 绘制无效窗口
                    if invalid_windows:
                        peak_hrs_invalid = [w['peak_hr_bpm'] for w in invalid_windows if w['peak_hr_bpm'] > 0]
                        true_hrs_invalid = [w['true_hr_bpm'] for w in invalid_windows if w['true_hr_bpm'] > 0]
                        if peak_hrs_invalid and true_hrs_invalid:
                            ax.scatter(peak_hrs_invalid, true_hrs_invalid, color='red', alpha=0.5, s=30,
                    ...
                    ax.fill_between(hr_range, 
                                   ...
                                   [h + self.hr_tolerance_bpm for h in hr_range],
                                   alpha=0.2, color='gray', label=f'±{self.hr_tolerance_bpm}BPM Tolerance')
                    
                    ax.set_xlabel('Peak Detection HR (BPM)')
                    ax.set_ylabel('True HR (BPM)')
    ...
    def process_single_subject(self, subject):
        """处理单个受试者（用于并行处理）"""
        try:
    ...
    def run_windowed_analysis(self, subject_list=None, n_jobs=2):
        """运行时域心率与真实值均值比较分析（并行版本）"""
        ...
        print(f"\n🔬 开始时域心率与真实值均值比较PTT分析（并行处理）")
        print(f"📋 受试者列表: {subject_list}")
        print(f"🔧 并行进程数: {n_jobs}")
        print(f"🎯 验证策略:")
        print(f"   - {self.window_duration}s窗口, {self.window_step}s滑窗步长")
        print(f"   - 时域峰值检测心率 vs 真实心率（从biopac文件获取）")
        print(f"   - 心率差异<{self.hr_tolerance_bpm}BPM才认为窗口有效")
        ...
        with mp.Pool(processes=n_jobs) as pool:
            results = list(tqdm(
                pool.imap(self.process_single_subject, subject_list),
        ...
        print(f"   7. hr_validation_exp_X.png - 时域心率与真实值对比图")
        print(f"\n🎯 时域心率与真实值均值比较完成！")
...
def main():
    """主函数"""
    print("🩺 时域心率与真实值均值比较PTT峰值检测器（并行版本）")
    print("=" * 70)
    print("📖 时域心率与真实值均值比较实现:")
    print("   • 20秒窗口，5秒滑窗步长")
    print("   • 时域峰值检测心率 vs 真实心率（从biopac文件获取）")
    print("   • 心率差异<5BPM才认为窗口有效")
    print("   • 只在有效窗口内计算PTT")
    print("   • 并行处理加速（2个进程）")
    print("=" * 70)
    ...
    # 使用并行处理，2个进程
    results = detector.run_windowed_analysis(n_jobs=8)
    
    # 如果需要处理特定受试者列表，可以取消注释下面的代码
    ...
    # results = detector.run_windowed_analysis(subject_list=subject_list, n_jobs=2)
    
    print("\n🎯 分析完成，时域心率与真实值均值比较已实现:")
    print("1. 检查window_validation_exp_X.csv了解每个窗口的验证状态")
    print("2. 查看hr_validation_exp_X.png确认时域心率与真实值一致性")
