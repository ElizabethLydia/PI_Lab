...
class PTTBloodPressureAnalyzer:
    ...
    def load_ground_truth_bp(self, exp_id):
        ...
        try:
            ...
            # 检查是否有校准血压数据列
            if 'moved_sbp' in df.columns and 'moved_dbp' in df.columns:
                # 重命名列以匹配生理指标映射
                df = df.rename(columns={
                    'moved_sbp': 'systolic_bp',
                    'moved_dbp': 'diastolic_bp'
                })
                
                # 计算平均动脉压 (MAP = DBP + 1/3(SBP-DBP))
                df['mean_bp'] = df['diastolic_bp'] + (1/3) * (df['systolic_bp'] - df['diastolic_bp'])
                
                print(f"📊 校准后的血压数据: SBP {len(df['systolic_bp'].dropna())} 点, DBP {len(df['diastolic_bp'].dropna())} 点, MAP {len(df['mean_bp'].dropna())} 点")
            else:
                print(f"⚠️ 未找到校准血压数据列，尝试使用原始列名")
...
def main():
    ...
    for subject in subject_list:
        ...
        analyzer.csv_output_dir = os.path.join(root_path, subject, 'csv_output_calibrated')
