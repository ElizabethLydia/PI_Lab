...
#     # if os.listdir(output_dir):  # 注释掉检查，强制重新处理
#     #     print(f"\nsubject {subject} 已处理（output目录非空），跳过")
#     #     return None  # 成功返回 None
...
#     start_time = time.time()
#     # manager = Manager()  # 注释掉
#     # failed_subjects = manager.list()  # 注释掉
...
#             res = process_subject(date_folder, subject)
#             results.append(res)
    
#     # failed_subjects = [res for res in results if res is not None]  # 注释掉
    
#     # if failed_subjects:  # 注释掉
#     #     with open('/root/autodl-tmp/failed_subjects.txt', 'w') as f:  # 注释掉
#     #         f.write("失败的 subject:\n")  # 注释掉
#     #         for fs in failed_subjects:  # 注释掉
#     #             f.write(f"{fs}\n")  # 注释掉
#     #     print(f"失败 subject 数量: {len(failed_subjects)}, 详情见 /root/autodl-tmp/failed_subjects.txt")  # 注释掉
#     # else:  # 注释掉
#     #     print("所有 subject 处理成功！")  # 注释掉
...
#     print(f"{'='*60}")
#     print(f"总处理耗时: {total_time:.1f} 秒")
#     print(f"处理了 {len(target_subjects)} 个指定subjects")
