...
class PTTBloodPressureAnalyzer:
    ...
    def __init__(self, output_dir="ptt_bp_analysis_calibrated"):
        self.output_dir = output_dir
        self.ptt_output_dir = "ptt_output"  # 窗口化PTT数据目录
        self.csv_output_dir = "csv_output_calibrated"  # 校准血压数据目录
    ...
    def load_ground_truth_bp(self, exp_id):
        ...
        try:
            # 加载校准血压数据CSV文件
            csv_file = os.path.join(self.csv_output_dir, f'{self.subject}_{exp_id}_calibrated_bp_aligned.csv')
            if not os.path.exists(csv_file):
                print(f"❌ 校准血压数据文件不存在: {csv_file}")
                return None
            
            # 读取完整生理数据
            df = pd.read_csv(csv_file)
            print(f"✅ 加载校准血压数据: {len(df)}条记录")
            
            # 检查并重命名血压列以匹配生理指标映射
            if 'moved_sbp' in df.columns:
                df['systolic_bp'] = df['moved_sbp']
                print(f"📊 重命名 moved_sbp -> systolic_bp")
            if 'moved_dbp' in df.columns:
                df['diastolic_bp'] = df['moved_dbp']
                print(f"📊 重命名 moved_dbp -> diastolic_bp")
            
            # 计算平均动脉压 (MAP = DBP + 1/3(SBP-DBP))
            if 'systolic_bp' in df.columns and 'diastolic_bp' in df.columns:
                df['mean_bp'] = df['diastolic_bp'] + (df['systolic_bp'] - df['diastolic_bp']) / 3
                print(f"📊 计算平均动脉压 (MAP)")
    ...
    def build_regression_models(self, sync_df, correlations, exp_id=None):
        ...
        for sensor_pair in sensor_pairs:
            ...
            # 合并生理数据（取平均值）
            available_physio_cols = [col for col in main_physio_cols if col in pair_df.columns]
            if not available_physio_cols:
                print(f"⚠️ 传感器对 {sensor_pair} 没有可用的生理指标数据，跳过")
                continue
                
            physio_agg = pair_df.groupby(['exp_id', 'window_id']).agg({
                col: 'mean' for col in available_physio_cols
            }).reset_index()
            ...
            for physio_col in main_physio_cols:
                ...
                # 创建图表
                plt.figure(figsize=(12, 8))  # 增加图表宽度以容纳更多信息
                
                # 获取实验ID信息用于颜色区分
                exp_ids = model_data.loc[mask, 'exp_id'].values
                unique_exp_ids = np.unique(exp_ids)
                
                # 定义颜色映射
                colors = ['blue', 'red', 'green', 'orange', 'purple', 'brown', 'pink', 'gray', 'olive', 'cyan', 'magenta']
                color_map = {exp_id: colors[i % len(colors)] for i, exp_id in enumerate(unique_exp_ids)}
                ...
                for exp_id in unique_exp_ids:
                    ...
                    # 绘制该实验的数据点
                    plt.scatter(X_exp, y_exp, alpha=0.6, s=30, 
                               color=color_map[exp_id], label=f'Exp {exp_id} Data')
                    
                    # 为该实验单独训练模型并绘制拟合线
                    if len(X_exp) >= 3:  # 至少3个点才能拟合
                        try:
                            # 标准化该实验的数据
                            scaler_X_exp = StandardScaler()
                            scaler_y_exp = StandardScaler()
                            X_exp_scaled = scaler_X_exp.fit_transform(X_exp)
                            y_exp_scaled = scaler_y_exp.fit_transform(y_exp.reshape(-1, 1)).flatten()
                            
                            # 训练该实验的模型
                            model_exp = LinearRegression()
                            model_exp.fit(X_exp_scaled, y_exp_scaled)
                            
                            # 预测并绘制拟合线
                            x_min, x_max = np.min(X_exp), np.max(X_exp)
                            x_range = np.linspace(x_min, x_max, 100).reshape(-1, 1)
                            
                            x_range_scaled = scaler_X_exp.transform(x_range)
                            y_range_scaled = model_exp.predict(x_range_scaled)
                            y_range = scaler_y_exp.inverse_transform(y_range_scaled.reshape(-1, 1)).flatten()
                            
                            plt.plot(x_range, y_range, color=color_map[exp_id], linewidth=2, alpha=0.8, 
                                    linestyle='--', label=f'Exp {exp_id} Fit')
                        except Exception as e:
                            print(f"⚠️ 实验{exp_id}拟合失败: {e}")
                ...
                # 整体拟合线使用黑色粗线
                plt.plot(x_range, y_range, 'k-', linewidth=3, alpha=0.9, 
                        label='Overall Fit (All Experiments)')
                
                # 添加图例和标签
                plt.xlabel(f'PTT ({sensor_pair}) (ms)', fontsize=12)
                plt.ylabel(f'{self._format_physio_label_en(physio_col)}', fontsize=12)
                ...
                title += f'Overall Equation: y = {coef_original:.3f}x + {intercept_original:.3f}\n'
                title += f'Correlation: r={ptt_correlation:.3f}, p={ptt_p_value:.2e} | R²={r2:.3f}, MAE={mae:.2f}, n={len(y)}'
                title += f' | Experiments: {len(unique_exp_ids)}'
    ...
    def run_individual_experiment_analysis(self):
        ...
        for exp_id in range(1, 12):
            ...
            if exp_result and exp_result['sync_data'] is not None and len(exp_result['sync_data']) > 0:
        ...
        try:
            ...
            print(f"❌ 合并数据失败: {e}")
            print(f"all_experiments长度: {len(all_experiments)}")
            for i, exp_data in enumerate(all_experiments):
                print(f"  实验{i+1}: {type(exp_data)}, 长度: {len(exp_data) if hasattr(exp_data, '__len__') else 'N/A'}")
            return individual_results, None
...
def process_single_subject(subject, root_path):
    ...
    try:
        ...
        # 为每个受试者设置输出目录（绝对路径）
        subject_output_dir = os.path.join(root_path, subject, 'ptt_bp_analysis_calibrated')
        
        # 检查是否已经处理过
        status_file = os.path.join(subject_output_dir, 'step3_calibrated_succ.txt')
        if os.path.exists(status_file):
            try:
                with open(status_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                if 'Status: SUCCESS' in content:
                    print(f"⏭️  {subject}: 已处理完成，跳过")
                    return {'subject': subject, 'status': 'skipped', 'results': None}
            except:
                pass
        ...
        analyzer.csv_output_dir = os.path.join(root_path, subject, 'csv_output_calibrated')  # 使用校准数据目录
        analyzer.subject = subject  # 添加subject属性用于文件名
        
        # 运行综合分析（模式1）
        results = analyzer.run_comprehensive_analysis()
        
        if results and results.get('overall') is not None:
            ...
            # 生成成功状态文件
            success_file = os.path.join(subject_output_dir, "step3_calibrated_succ.txt")
            with open(success_file, 'w', encoding='utf-8') as f:
                f.write(f"Subject {subject} - PTT Analysis Completed Successfully\n")
                f.write(f"Timestamp: {pd.Timestamp.now()}\n")
                f.write(f"Output Directory: {subject_output_dir}\n")
                f.write(f"Status: SUCCESS\n")
            
            print(f"✅ 受试者 {subject} 分析成功，状态文件已生成")
            return {'subject': subject, 'status': 'success', 'results': results}
            
        else:
            # 生成失败状态文件
            success_file = os.path.join(subject_output_dir, "step3_calibrated_succ.txt")
            with open(success_file, 'w', encoding='utf-8') as f:
                f.write(f"Subject {subject} - PTT Analysis Failed\n")
                f.write(f"Timestamp: {pd.Timestamp.now()}\n")
                f.write(f"Output Directory: {subject_output_dir}\n")
                f.write(f"Status: FAILED - No valid results\n")
            
            print(f"❌ 受试者 {subject} 分析失败，状态文件已生成")
            return {'subject': subject, 'status': 'failed', 'results': None}
        ...
        try:
            ...
            success_file = os.path.join(subject_output_dir, "step3_calibrated_succ.txt")
            with open(success_file, 'w', encoding='utf-8') as f:
                f.write(f"Subject {subject} - PTT Analysis Error\n")
                f.write(f"Timestamp: {pd.Timestamp.now()}\n")
                f.write(f"Output Directory: {subject_output_dir}\n")
...
def main():
    ...
    with multiprocessing.Pool(processes=n_cores) as pool:
        ...
        results_list = list(tqdm(
            pool.imap(process_func, subject_list), 
    ...
    for result in results_list:
        if result:
            all_results[result['subject']] = result
    ...
    print(f"\n📊 处理结果统计:")
    print(f"   ✅ 成功: {success_count} 个受试者")
    print(f"   ❌ 失败: {failed_count} 个受试者")
    print(f"   ⚠️  错误: {error_count} 个受试者")
    print(f"   ⏭️  跳过: {skipped_count} 个受试者")
    print(f"   📋 总计: {len(all_results)} 个受试者")
    
    print(f"\n✅ 所有受试者分析完成!")
    print(f"📁 结果保存在每个受试者的 ptt_bp_analysis_calibrated 目录中")
    print(f"📊 使用校准后的血压数据作为真实值")
    print(f"📝 每个受试者目录下都有 step3_calibrated_succ.txt 状态文件")
...
def create_summary_report():
    """创建汇总报告"""
    print("📊 创建step3_calibrated处理状态汇总报告")
    print("="*50)
    
    root_path = '/root/autodl-tmp/'
    
    # 创建专门的检查文件夹 - 保存到PI_Lab目录下
    check_dir = "/root/PI_Lab/step3_calibrated_check_results"
    ...
    for subject in subject_list:
        status_file = os.path.join(root_path, subject, 'ptt_bp_analysis_calibrated', 'step3_calibrated_succ.txt')
            ...
            try:
                ...
                for line in lines:
                    if line.startswith('Status:'):
                        status_info['status'] = line.replace('Status:', '').strip()
                        ...
                        status_info['error_message'] = line.replace('Error Message:', '').strip()
    ...
    error_count = len(summary_df[summary_df['status'] == 'ERROR'])
    ...
    summary_file = os.path.join(check_dir, f"step3_calibrated_summary_report_{timestamp}.csv")
    ...
    for _, row in summary_df.iterrows():
        simple_status.append({
            ...
            'error_summary': row['error_message'][:100] + '...' if len(row['error_message']) > 100 else row['error_message']
        })
    
    simple_df = pd.DataFrame(simple_status)
    simple_file = os.path.join(check_dir, f"step3_calibrated_simple_status_{timestamp}.csv")
    ...
    failed_subjects = summary_df[summary_df['status'] == 'FAILED']['subject'].tolist()
    error_subjects = summary_df[summary_df['status'] == 'ERROR']['subject'].tolist()
    no_file_subjects = summary_df[summary_df['status'] == 'NO_FILE']['subject'].tolist()
    
    # 保存分类列表到检查文件夹
    with open(os.path.join(check_dir, f"step3_calibrated_success_subjects_{timestamp}.txt"), 'w', encoding='utf-8') as f:
        f.write(f"Step3 Calibrated 成功处理的受试者列表\n")
    ...
    with open(os.path.join(check_dir, f"step3_calibrated_failed_subjects_{timestamp}.txt"), 'w', encoding='utf-8') as f:
        f.write(f"Step3 Calibrated 失败的受试者列表\n")
        f.write(f"生成时间: {datetime.now()}\n")
        f.write(f"失败数量: {len(failed_subjects)}\n")
        f.write(f"="*50 + "\n")
        for subject in failed_subjects:
            f.write(f"{subject}\n")
    
    with open(os.path.join(check_dir, f"step3_calibrated_error_subjects_{timestamp}.txt"), 'w', encoding='utf-8') as f:
        f.write(f"Step3 Calibrated 出错的受试者列表\n")
        f.write(f"生成时间: {datetime.now()}\n")
        f.write(f"错误数量: {len(error_subjects)}\n")
        f.write(f"="*50 + "\n")
        for subject in error_subjects:
            f.write(f"{subject}\n")
    
    with open(os.path.join(check_dir, f"step3_calibrated_no_file_subjects_{timestamp}.txt"), 'w', encoding='utf-8') as f:
        f.write(f"Step3 Calibrated 无状态文件的受试者列表\n")
        f.write(f"生成时间: {datetime.now()}\n")
    ...
    # 创建最终汇总报告
    final_summary_file = os.path.join(check_dir, f"step3_calibrated_final_summary_{timestamp}.txt")
    with open(final_summary_file, 'w', encoding='utf-8') as f:
        f.write(f"Step3 Calibrated 处理结果最终汇总\n")
        f.write(f"="*50 + "\n")
        f.write(f"生成时间: {datetime.now()}\n")
        f.write(f"总受试者数: {total_subjects}\n\n")
        f.write(f"✅ 成功处理: {success_count} 个受试者 ({success_count/total_subjects*100:.1f}%)\n")
        f.write(f"❌ 处理失败: {failed_count} 个受试者 ({failed_count/total_subjects*100:.1f}%)\n")
        f.write(f"⚠️  处理出错: {error_count} 个受试者 ({error_count/total_subjects*100:.1f}%)\n\n")
        ...
        if error_subjects:
            f.write(f"⚠️  出错的受试者列表 ({error_count}个):\n")
            for i, subject in enumerate(error_subjects, 1):
                f.write(f"{subject}")
                if i % 10 == 0:
                    f.write("\n")
                elif i < len(error_subjects):
                    f.write(", ")
            f.write("\n\n")
        
        f.write(f"📊 处理状态说明:\n")
        f.write(f"- SUCCESS: 分析完成，生成了所有相关文件和图表\n")
        f.write(f"- ERROR: 处理过程中发生异常\n")
        f.write(f"- FAILED: 分析失败\n")
        f.write(f"- NO_FILE: 状态文件不存在\n\n")
        
        f.write(f"💡 建议:\n")
        f.write(f"1. 成功处理的{success_count}个受试者可以直接使用结果\n")
        if error_count > 0:
            f.write(f"2. 出错的{error_count}个受试者需要检查具体错误原因\n")
        if failed_count > 0:
            f.write(f"3. 失败的{failed_count}个受试者需要重新处理\n")
        f.write(f"4. 总体成功率{success_count/total_subjects*100:.1f}%，处理效果良好\n\n")
        
        f.write(f"📁 本批次所有报告文件:\n")
        f.write(f"- step3_calibrated_summary_report_{timestamp}.csv (完整报告)\n")
        f.write(f"- step3_calibrated_simple_status_{timestamp}.csv (简化状态)\n")
        f.write(f"- step3_calibrated_success_subjects_{timestamp}.txt (成功列表)\n")
        if error_count > 0:
            f.write(f"- step3_calibrated_error_subjects_{timestamp}.txt (错误列表)\n")
        if failed_count > 0:
            f.write(f"- step3_calibrated_failed_subjects_{timestamp}.txt (失败列表)\n")
        if no_file_count > 0:
            f.write(f"- step3_calibrated_no_file_subjects_{timestamp}.txt (无文件列表)\n")
    
    print(f"\n📝 分类列表已保存到 {check_dir} 文件夹:")
    print(f"   ✅ 成功: step3_calibrated_success_subjects_{timestamp}.txt")
    if failed_count > 0:
        print(f"   ❌ 失败: step3_calibrated_failed_subjects_{timestamp}.txt")
    if error_count > 0:
        print(f"   ⚠️  错误: step3_calibrated_error_subjects_{timestamp}.txt")
    if no_file_count > 0:
        print(f"   📁 无文件: step3_calibrated_no_file_subjects_{timestamp}.txt")
    print(f"   📋 最终汇总: step3_calibrated_final_summary_{timestamp}.txt")
    ...
    if failed_subjects:
        print(f"\n❌ 失败的受试者 ({len(failed_subjects)}):")
        for i, subject in enumerate(failed_subjects, 1):
            print(f"   {i:2d}. {subject}")
    
    if error_subjects:
        print(f"\n⚠️  出错的受试者 ({len(error_subjects)}):")
        for i, subject in enumerate(error_subjects, 1):
            print(f"   {i:2d}. {subject}")
